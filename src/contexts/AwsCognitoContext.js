...
import { encodePHPhoneNumber, capitalizerOfWords } from 'utils/common/stringHelper';
import { createUser } from 'redux/slices/user/actions';
// utils
import axios from '../utils/axios';
// routes
import { PATH_AUTH } from '../routes/paths';
//
import { COGNITO_API, PATH_AFTER_LOGIN } from '../config';
...
function AuthProvider({ children }) {
  ...
  const register = (email, phoneNumber, password, firstName, lastName) => {
    ...
    return new Promise((resolve, reject) =>
      UserPool.signUp(
        ...
        [
          ...
          { Name: 'name', Value: `${capitalizerOfWords(firstName)} ${capitalizerOfWords(lastName)}` },
          { Name: 'family_name', Value: `${capitalizerOfWords(lastName)}` },
          { Name: 'given_name', Value: `${capitalizerOfWords(firstName)}` },
  ...
  const confirmRegistration = async (email, code) => {
    const userData = {
      Username: 'princejoeylee12@gmail.com',
      Pool: UserPool,
    };
  ...
  const resendConfirmation = async (email, code) => {
    const userData = {
      Username: email,
      Pool: UserPool,
    };
    const cognitoUser = new CognitoUser(userData);
    return cognitoUser.resendConfirmationCode(code, true, (err, result) => {
      if (err) {
        return false;
      }
      return true;
